...
const CarrierPricing = props => {
  ...
  useEffect(() => {
    (async () => {
      try {
        await contextValue.db.collection('pricing').doc('carrier-pricing').
          onSnapshot(
            function(doc){
              setValues(doc.data())
            }
  ...
  const handleClick = async () => {
    //console.log(values);
    await contextValue.db.collection('pricing').doc("carrier-pricing").update(values).then(()=>setShowUpdated(true))
  }
